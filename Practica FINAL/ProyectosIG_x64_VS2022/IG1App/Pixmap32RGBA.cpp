...
struct bitmap_file_header
{
	...
	/*DWORD*/
	unsigned int size_file; // unreliable ->  = (offset_map + size_map) bytes 
	/*WORD*/
	unsigned short reserved1; // 0
	/*WORD*/
	unsigned short reserved2; // 0
	/*DWORD*/
	unsigned int offset_map; // 54 (14 fileheader + 40 information_header) bytes 
};

struct bitmap_information_header
{
	/*DWORD*/
	unsigned int size_info_header; // 40 bytes = sizeof(bitmap_information_header)
	/*LONG*/
	int width; // columns (left to right order)
	/*LONG*/
	int height; // rows (bottom to top order)
	/*WORD*/
	unsigned short planes; // 1 color planes 
	/*WORD*/
	unsigned short bits_per_pixel; // 24 bits -> 3(BGR) bytes
	/*DWORD*/
	unsigned int compression; // 0 = BI_RGB, no pixel array compression used
	/*DWORD*/
	unsigned int size_map; // (width * 3 + pading) * height  bytes
	/*LONG*/
	int x_prpm; // (Print resolution pixels/meter horizontal) unreliable
	/*LONG*/
	int y_prpm; // (Print resolution pixels/meter vertical) unreliable 
	/*DWORD*/
	unsigned int clr_used; // 0 (Number of colors in the palette)
	/*DWORD*/
	unsigned int clr_important; // 0 (means all colors are important)
};

//-------------------------------------------------------------------------------------------

void PixMap32RGBA::load_headers(std::ifstream& stream, unsigned int& width, unsigned int& height) const
...
void PixMap32RGBA::load_bmp24BGR(const std::string& file_name) // throw(std::ios_base::failure, std::bad_alloc)
{
	...
	auto rowAux = new char [rowSize]; // GLubyte
